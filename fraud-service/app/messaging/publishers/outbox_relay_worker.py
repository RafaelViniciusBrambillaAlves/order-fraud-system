    ...
    async def _publish_message(self, channel, msg) -> None:
        ...
        with tracer.start_as_current_span(
            ...
            try:
                ...
                await exchange.publish(amqp_message, routing_key=msg.routing_key)
                ...
                fraud_metrics.outbox_messages_published_total.add(
                ...
                # FIX: string de formato unificada em uma única chamada
                logger.error(
                    "Outbox publish failed | id=%s event=%s attempt=%d duration_ms=%.1f error=%s",
                    msg.id,
                    msg.event_type,
                    msg.retry_count + 1,
                    publish_elapsed * 1000,
